...
export function Field({
  ...
  return (
    <div className="space-y-2">
      ...
      {children}
      {hint && !error ? <p className="text-brand-700/80 text-sm">{hint}</p> : null}
      {error ? (
  ...
  return (
    <input
      className={cn(
        ...
        'rounded-field border-2 bg-white px-3 transition-all duration-200 ease-out outline-none',
        'focus:border-accent-500 aria-invalid:border-danger-500 disabled:opacity-60',
        className,
      )}
      {...props}
    />
  ...
  return (
    <div
      className={cn(
        ...
        'overflow-hidden border-2 bg-white transition-all duration-200 ease-out',
        className,
      )}
    >
      {icon ? (
